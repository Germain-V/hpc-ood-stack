locals {
  ...
  instance_pool_image = !var.cluster_network && var.use_marketplace_image ? try(oci_core_app_catalog_subscription.mp_image_subscription[0].listing_resource_id, local.image_ocid) : local.image_ocid
  ...
  timeout_per_batch = var.cluster_network ? 60 : 30
  ...
  // IDCS
  idcs_url  = var.use_existing_idcs ? data.oci_identity_domain.idcs_domain[0].url : oci_identity_domain.new_idcs_domain[0].url

  idcs_domain_id   = var.use_existing_idcs ? data.oci_identity_domain.idcs_domain[0].id : oci_identity_domain.new_idcs_domain[0].id

  idcs_display_name = var.use_existing_idcs ? data.oci_identity_domain.idcs_domain[0].display_name : oci_identity_domain.new_idcs_domain[0].display_name

  idcs_description = var.use_existing_idcs ? data.oci_identity_domain.idcs_domain[0].description : oci_identity_domain.new_idcs_domain[0].description
}

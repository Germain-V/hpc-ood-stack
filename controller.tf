...
resource "oci_core_instance" "controller" {
  depends_on          = [data.oci_core_subnet.public_subnet]
  availability_domain = var.controller_ad
  compartment_id      = var.targetCompartment
...
resource "null_resource" "controller" {
  depends_on = [oci_core_instance.controller, oci_core_volume_attachment.controller_volume_attachment, oci_identity_domains_app.ood_app]
  triggers = {
    controller     = oci_core_instance.controller.id
    scripts_hash   = md5(join("", [for f in fileset(path.module, "scripts/**"): filemd5("${path.module}/${f}")]))
    playbooks_hash = md5(join("", [for f in fileset(path.module, "playbooks/**"): filemd5("${path.module}/${f}")]))
    conf_hash      = md5(join("", [for f in fileset(path.module, "conf/**"): filemd5("${path.module}/${f}")]))
  }

  #### Provision OOD scripts
  provisioner "file" {
    source      = "scripts/ood.sh"
    destination = "ood.sh"
    connection {
      host        = local.host
      type        = "ssh"
  ...
  provisioner "file" {
    source      = "scripts/customize_ood_ui.sh"
    destination = "customize_ood_ui.sh"
    connection {
      host        = local.host
      type        = "ssh"
      user        = var.controller_username
  ...
  # Render and upload OOD environment configuration
  provisioner "file" {
    content     = templatefile("${path.module}/conf/ood.env.tpl", {
      ood_dns_q           = local.ood_public_dns,
      ood_username_q      = var.ood_username,
      ood_user_password_q = var.ood_user_password,
      ood_user_email_q    = var.ood_user_email,
      client_id_q         = oci_identity_domains_app.ood_app.name,
      client_secret_q     = oci_identity_domains_app.ood_app.client_secret,
      idcs_url_q          = local.idcs_url
    })
    destination = "/home/${var.controller_username}/ood.env"
    connection {
      host        = local.host
      type        = "ssh"
      user        = var.controller_username
  ...
  # Ensure /opt/oci-hpc exists and is writable by the SSH user before uploading into it
  ...
  # Run OOD installation after all required files/directories are uploaded, to avoid SSH disconnects mid-transfer
  provisioner "remote-exec" {
    inline = [
      "#!/bin/bash",
      "sudo mkdir -p /opt/oci-hpc",
      "sudo chown ${var.controller_username}:${var.controller_username} /opt/oci-hpc/",
      "sudo mv /home/${var.controller_username}/ood.env /opt/oci-hpc/ood.env || true",
      "sudo chown ${var.controller_username}:${var.controller_username} /opt/oci-hpc/ood.env || true",
      "mkdir -p /opt/oci-hpc/bin",
      "mkdir -p /opt/oci-hpc/playbooks",
      "chmod +x ood.sh",
      "chmod +x cert-install.sh",
      "MAX_RETRIES=5",
      "for i in {1..$MAX_RETRIES}; do",
      "  echo ${local.idcs_url}",
      "  timeout --foreground 45m sudo ./ood.sh",
      "  if [ $? -eq 0 ]; then break; fi",
      "  echo 'Retrying certbot in 5 seconds...'",
      "  sleep 5",
      "done",
      "echo 'Customizing UI'",
      "chmod +x customize_ood_ui.sh",
      "sudo ./customize_ood_ui.sh",
    ]
    on_failure = continue
    connection {
      host        = local.host
...
resource "null_resource" "inventory_upload" {
  depends_on = [null_resource.controller]
  triggers = {
    inventory_hash   = md5(templatefile("${path.module}/inventory.tpl", {
...
resource "null_resource" "cluster" {
  depends_on = [null_resource.controller, null_resource.inventory_upload, null_resource.backup, oci_core_compute_cluster.compute_cluster, oci_core_cluster_network.cluster_network, oci_core_instance.controller, oci_core_volume_attachment.controller_volume_attachment]
  triggers = {
    scripts_hash    = md5(join("", [for f in fileset(path.module, "scripts/**"): filemd5("${path.module}/${f}")]))
    playbooks_hash  = md5(join("", [for f in fileset(path.module, "playbooks/**"): filemd5("${path.module}/${f}")]))
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "timeout --foreground 2h /opt/oci-hpc/bin/configure.sh 2>&1 | tee /opt/oci-hpc/logs/initial_configure.log",
      "exit_code=$${PIPESTATUS[0]}",
      "timeout --foreground 8m /opt/oci-hpc/bin/initial_monitoring.sh || true",
    "exit $exit_code"]
    on_failure = continue
    connection {
      host        = local.host
...
locals {
  ...
  # ODD DNS
  ood_public_dns = "ood-${replace(oci_core_instance.controller.public_ip, ".", "-")}.nip.io"
}
/*

...
## SELinux and Open OnDemand (OOD)

When SELinux is enforcing (recommended), Open OnDemand (Apache front-end + per-user NGINX PUN) needs specific labels, booleans and a small policy so Apache can connect to each user’s PUN Unix socket. This stack configures these automatically during provisioning.

### What this stack configures automatically
- Persistent file contexts (survive relabels/reboots):
  - `/run/ondemand-nginx(/.*)?` and `/var/run/ondemand-nginx(/.*)?` labeled for OOD PUN runtime
  - `/var/lib/ondemand-nginx(/.*)?` and `/var/log/ondemand-nginx(/.*)?` labeled writable by httpd
- One‑shot service to restore contexts at boot: `restorecon-ood.service`
- Boot-time creation of `/run/ondemand-nginx` via `tmpfiles.d`
- SELinux policy module: allow `httpd_t` to connect to PUN sockets (`connectto` on OOD PUN sock_file type)
- Booleans: `httpd_can_network_connect=on`, `httpd_setrlimit=on`
- Ensure PAM helper is correctly labeled: `restorecon /usr/sbin/unix_chkpwd`
- Ensure `auditd` is enabled (sudo/PAM auditing)

These steps are implemented in `scripts/ood.sh` and applied by Terraform on the controller.

### How OOD works (paths and labels)
- Apache (httpd) fronts the portal; each user gets a PUN (per‑user NGINX) process.
- PUN runtime socket: `/var/run/ondemand-nginx/<user>/passenger.sock` (created on first login)
- PUN logs/config: `/var/log/ondemand-nginx/<user>/nginx/*`, `/var/lib/ondemand-nginx/<user>`
- `/run` is ephemeral; the parent dir is re-created at boot; per-user subdirs appear when the PUN starts.

### Verify after deploy or reboot
```bash
# Module/booleans
semodule -l | grep ood_httpd_sock
getsebool httpd_can_network_connect httpd_setrlimit

# Parent dir label
ls -Zd /var/run/ondemand-nginx

# After first user access to OOD, the user socket should exist and be labeled appropriately
ls -Z /var/run/ondemand-nginx/<user>/passenger.sock
```

### Troubleshooting
- Check AVC denials and generate a minimal policy if needed:
```bash
ausearch -m avc -ts recent | audit2allow -M ood_extra && semodule -i ood_extra.pp
```
- If sudo/PAM shows “account management error” after reboot, confirm `auditd` is active and SELinux booleans above are set.
- If the PUN fails to start and the user socket is missing, create the user runtime dir to expose AVCs:
```bash
install -d -m 0755 -o <user> -g <user> /var/run/ondemand-nginx/<user>
restorecon -Rv /var/run/ondemand-nginx/<user>
/opt/ood/nginx_stage/sbin/nginx_stage nginx --user <user>
tail -n 200 /var/log/ondemand-nginx/<user>/nginx/error.log
ausearch -m avc -ts recent | audit2allow
```

### Security note
- Do not grant `httpd_t` direct read access to `shadow_t`. PAM should use `/usr/sbin/unix_chkpwd` (properly labeled) to access shadow securely.

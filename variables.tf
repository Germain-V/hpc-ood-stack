...
variable "ood_username" {       #Provide the username for OOD
  type = string
  default = "ood_user"
}
...
variable "use_existing_idcs" {
  description = "Whether to use an existing IDCS domain (true) or create a new one (false)"
  type        = bool
  default     = false
}

variable "existing_domain_ocid" {
  description = "OCID of the existing IDCS domain"
  type        = string
  default     = ""
}

variable "ood_display_name" {
  description = "Please use underscores or dashes instead of spaces"
  type        = string
  default = "ood_app"
}
